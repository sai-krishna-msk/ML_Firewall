...
def searchAPI(item):
    g.db = connect_db()
    print(item)
    result = test.test(item)
    query ="SELECT * FROM shop_items WHERE name LIKE '{}' ".format(item)
    f = open("test.sql", "a")
    f.writelines(query+"\n")



    #curs = g.db.execute("SELECT * FROM shop_items WHERE name=?", item) #The safe way to actually get data from db withoutml
    ...
    print(results)
    if(result ==1):
        results = [dict(name="its an", quantity="SQL", price="Injection")]
    print(results)
